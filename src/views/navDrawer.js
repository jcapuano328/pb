...
let NavigationDrawer = React.createClass({    
    onSelect(e) {
        let g = Games.game(e);
        this.props.reset(g);
        Actions.game({title: g.name, subtitle: g.subtitle});
    },    
    render () {
        return (            
            <NavDrawer items={Games.games.map((g) => ({key:g.id,name:g.name,desc:'',image:Icons[g.image]}))} icons={Icons} onSelect={this.onSelect} >
                {this.props.children}
            </NavDrawer>                
        );
    }
